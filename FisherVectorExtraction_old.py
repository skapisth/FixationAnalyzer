"""
Code from - https://github.com/jonasrothfuss/fishervector/blob/master/fishervector/FisherVector.py
"""
import numpy as np
...
class FisherVectorExtraction(object):
  ...
  def predict(self, X, normalized=True):
    ...
    if X.ndim == 4:
      ...
      return np.reshape(result,(orig_shape[0],orig_shape[-1]*2*self.n_kernels))#np.reshape(result, (orig_shape[0], 2 * self.n_kernels, orig_shape[-1]))
  ...
  def _fit(self, X, model_dump_path=None, verbose=True):
    ...
    self.gmm = gmm = GaussianMixture(n_components=self.n_kernels, covariance_type=self.covariance_type,reg_covar=self.reg_covar, max_iter=1000).fit(X)
